...
function filterItemsBook(items, filterFn) {
    return items.filter(filterFn);
}
const filteredItemsOfBook = (value) => {
    return value.type === ItemType.Book
        ? value.author === "Harper Lee"
            ? true
            : false
        : false;
};
const result = filterItemsBook(libraryItems, filteredItemsOfBook);
console.log(result);
